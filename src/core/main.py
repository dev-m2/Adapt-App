...
def reviewAdaptations():
    def _review(adapt_id: str) -> bool:
        ...
        # Special case: "bar" type cards launch the interactive pygame bar training game.
        # The game session will handle *all* currently due bar cards (multiple customers),
        # apply FSRS grade 3 for successfully fulfilled orders and grade 2 for unsuccessful ones,
        # show "All cards reviewed..." when the due pool is exhausted, then allow practice.
        # This matches the requested usage cycle (import .nm -> due bar cards -> game on review).
        if isinstance(content, dict) and content.get("type") == "bar":
            try:
                from ..bar.game import run_bar_review_session
                print("\nLaunching bar training game for due bar card(s)...")
                run_bar_review_session()
                print("Bar review session complete. Returning to main review loop.")
                return True
            except Exception as e:
                print(f"Could not launch bar review session: {e}")
                return False
